...
sigm2 = input(['Assumed noise variance [mad = ',num2str(mad(Mn(:))),'] sigm =  '], 's');
     %sigm2 = input(['Assumed noise variance [mad = ',num2str(mad(Mn(:),1)/0.6745),'] sigm =  '], 's');
%      sigm2 = input(['Assumed noise variance [mad = ',num2str(ENVSM(Mn)),'] sigm =  '], 's');
    if isempty(sigm2)
        sigm2 = mad(Mn(:));
        %sigm2 = mad(Mn(:),1)/0.6745;
%         sigm2 = ENVSM(Mn);
    else
