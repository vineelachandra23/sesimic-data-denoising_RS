...
elseif isfield(options,'w') && not(isfield(options,'W'))
    ...
    %W = getoptions(options,'W',9);
    W = getoptions(options,'W',81);
%     W = getoptions(options,'W',64);
    %W = getoptions(options,'W',100);
...
%dico_sigm = getoptions(options,'dico_sigm',mad(Mn(:),1)/0.6745); % Assumed noise variance
% dico_sigm = getoptions(options,'dico_sigm',ENVSM(Mn)); % Assumed noise variance
...
%  T = getoptions(options,'T',2); % Sparsity target zhen using OMP
%  T = getoptions(options,'T',1); % Sparsity target zhen using OMP
